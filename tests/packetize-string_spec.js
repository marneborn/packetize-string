...
describe("Testing the packetize-string classes", function () {
    ...
    beforeEach(function () {
        packetSize =  2047; // defined in package, can't be changed.
        headerSize =     1;
    });
    ...
    describe("Making sure that the Sender class works", function () {
        ...
        it ('should be able to use Sender to create a packet', function () {
            ...
            let inMsg = "Hello World";
            sender.packetize(inMsg);
            expect(sender.hasPacket()).toBeTruthy();
            expect(sender.numPackets()).toBe(1);
            
            let packet = String.fromCharCode(inMsg.length + headerSize)+inMsg;
            let outMsg = sender.next();
        ...
        it ( 'should be able to break a long string into multiple packet', function () {
            ...
            let strSize    = 10000;
            let numPackets = Math.ceil(strSize/(packetSize-headerSize));
            let lastSize   = strSize - (numPackets-1) * (packetSize);
            
            // create a string that is 10,000 x's
            let inMsg      = Array(strSize+1).join('x');
            
            let packet1    = String.fromCharCode(packetSize)+Array(packetSize).join('x');
            let packetLast = String.fromCharCode(lastSize+1)+Array(lastSize).join('x');
            let packetRest = Array(numPackets-1).join(packet1)+packetLast; // string after 'getting' first packet.
            ...
            let restMsg  = sender.all();
            let restSize = strSize - (packetSize-headerSize) + (numPackets-1)*headerSize;
        ...
        it ( 'should send a second packet with zero lengthed payload if the message is 1 less than the limit', function () {
            ...
            let inMsg  = Array(packetSize-1+1).join('x');
            ...
            let outMsg1 = sender.next();
            expect(outMsg1.length).toEqual(packetSize);
            expect(sender.hasPacket()).toBeTruthy();
            expect(sender.numPackets()).toBe(1);
            
            let outMsg2 = sender.next();
        ...
        it ( 'should only send 1 packet if the message is 2 smaller than the limit', function () {
            ...
            let inMsg  = Array(packetSize-2+1).join('x');
    ...
    describe("Making sure that the Reader class works", function () {
        ...
        it ('should be able to use Reader to receive a packet and conver it to a message', function () {
            ...
            expect(receiver.hasMessage()).toBeFalsy();
            expect(receiver.numMessages()).toBe(0);

            let inMsg  = "Hello World";
            let packet = String.fromCharCode(inMsg.length+headerSize)+inMsg;

            receiver.accumulate(packet);
            expect(receiver.hasMessage()).toBeTruthy();
            expect(receiver.numMessages()).toBe(1);

            let outMsg = receiver.next();
            expect(receiver.hasMessage()).toBeFalsy();
            expect(receiver.numMessages()).toBe(0);
            expect(outMsg).toEqual(inMsg);
    ...
    describe("Making sure that the Senders and Receivers work together.", function () {
        ...
        it ("should be able to packetize a message with a Sender and use a Receiver to extract the message", function () {
            ...
            sender.packetize(inMsg);
            expect(sender.numPackets()).toBe(7);

            receiver.accumulate(sender.all());
            expect(receiver.numMessages()).toBe(1);

            let outMsg = receiver.next(); 
            expect(outMsg.length).toBe(inMsg.length);
        ...
        it("Should be able to packetize two separate long messages, accumulate random sized chunks, and get the same message out", function () {
            ...
            packets.push(sender.next());
            packets.push(sender.next());
            packets.push(sender.all());

            for ( var i=0; i<packets.length; i++ ) {
                let packet = packets[i];
                while ( packet !== "" ) {
                    let spot = Math.ceil(Math.random()*packet.length);
                    receiver.accumulate(packet.substr(0, spot));
                    packet = packet.substr(spot);
                }
            }

            var oMsg1 = receiver.next();
            expect(oMsg1.length).toEqual(longMsg1.length);
            expect(oMsg1).toEqual(longMsg1);

            var oMsg2 = receiver.next();
            expect(oMsg2.length).toEqual(longMsg2.length);
            expect(oMsg2).toEqual(longMsg2);
        });
    });
});
