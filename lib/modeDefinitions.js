...
const DEFAULT_MODE    = "_default_";
const DEFAULT_ID      = 0;
const ID_MSB          = 15;
const ID_LSB          = 11;
const ID_MASK         = parseInt("F800", 16);
const EXTRA_MASK      = parseInt("07FF", 16);
...
module.exports.encode = function ( info, string ) {
    ...
    let mode = (info.mode != null) ? info.mode : DEFAULT_MODE;
...
module.exports.decode = function ( info, buffer ) {
    ...
    if ( info.mode === null ) {
        ...
        let header = buffer.charCodeAt(0);
        let id     = header >>> ID_MSB;
...
let MAX_CHARS    = Math.pow(2, ID_LSB) - 2;
...
function defaultEncode ( string ) {
    ...
    while ( string != null ) {
        ...
        let packet = String.fromCharCode( (DEFAULT_ID << ID_MSB) + payload.length + 1 ) + payload;
        packets.push(packet);
    }
    return packets;
}
...
function defaultDecode ( info, buffer ) { 
    ...
    return {
        message : buffer.substr(1, length-1),
        rest    : buffer.substr(length)
    };
}
