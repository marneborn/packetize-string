...
function makeAndSend ( string, config ) {
    let obj = new Sender(string,config);
    return obj.toString();
}
...
function receivePromise ( socket ) {
    let defer = Q.defer();
    let rcv   = new Receiver();
    socket.on('data', function ( chunk ) { rcv.accumulate(chunk) } );
    rcv
        .on('error', function ( err ) {
            defer.reject(err);
            rcv.removeAllListeners();
        })
        .on('message', function ( msg ) {
            let leftOvers = rcv.leftOvers();
            defer.resolve(msg);
            rcv.removeAllListeners();
        });
